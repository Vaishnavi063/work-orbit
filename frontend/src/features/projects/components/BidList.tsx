...
const BidList: React.FC<BidListProps> = ({ bids, projectStatus, projectId }) => {
  ...
  const handleBidAction = async (bidId: number, action: "accept" | "reject") => {
    ...
    // Optimistic update
    dispatch(updateBidStatusOptimistic({ 
      bidId, 
    ...
    try {
      ...
      if (actionThunk.fulfilled.match(result)) {
        ...
        toast.error(
          ...
          {
            description: error.bidAction || "Please try again",
          }
        );
      }
    } catch (err) {
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="font-medium">{bid.teamSize} member{bid.teamSize !== 1 ? "s" : ""}</span>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <p className="text-muted-foreground max-w-md">
        ...
        Bids will appear here once freelancers start showing interest in your project.
      </p>
    </div>
  );
};
